...
const saveAndPlayAudio = async (id, url) => {
	try {
		...
		request.onsuccess = async (event) => {
			if (event.target.result) {
				...
				playAudio(event.target.result.audioData)
			} else {
				...
				newTransaction.oncomplete = () => playAudio(audioData)
...
const AudioPlayer = () => {
	...
	const audioUrls = [
		{id: 1, url: "https://testapi1test.blob.core.windows.net/media/0.m4a"},
		{id: 2, url: "https://testapi1test.blob.core.windows.net/media/1.m4a"},
		// Add more URLs as needed
	]

	const handleSaveAndPlayAudio = (id, url) => {
		console.log(db)
		saveAndPlayAudio(id, url)
	}
	...
	return (
		<>
			{audioUrls.map((audioUrl) => (
				<div key={audioUrl.id}>
					<button
						onClick={() => handleSaveAndPlayAudio(audioUrl.id, audioUrl.url)}
						// disabled={!isDbOpen}
					>
						Save and Play Audio {audioUrl.id}
					</button>
					<br />
					<br />
				</div>
